...
export default function CartItems() {
	...
	const totalPrice = cartItems.reduce((value, item) => value + item.price * item.qunatity, 0);
					...
					return (
						<li key={item.id}>
							...
							<div className="cart-item-actions">
								<button className="cart-item-actions min" onClick={() => handleRemoveFromCart(item.id)}>
									-
								</button>
								<span>{item.qunatity}</span>
								<button className="cart-item-actions max" onClick={() => handleAddToCart(item)}>
									+
								</button>
							</div>
						</li>
